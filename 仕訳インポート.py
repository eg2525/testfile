...
def ap():
    ...
    if uploaded_file is not None:
        ...
        for fixed_header in fixed_headers:
            col1, col2 = st.columns([1, 1])  # 画面を左側と右側に等しく分割
            with col1:
                st.text(fixed_header)  # 固定ヘッダー名を左側に表示
            with col2:
                # 対応するアップロードされたファイルのヘッダーを選択するためのプルダウンメニューを右側に表示
                selected_option = st.selectbox(f"Select column for: {fixed_header}", options, key=f"{fixed_header}_select")
